 ...
 * @author Huyen
 */
public class WoodManagerJDialog extends javax.swing.JDialog implements WoodController {

    
    WoodDao dao = new WoodDAOImpl();
    List<Wood> items = List.of();
    CategoryDao CategoryDao = new CategoryDAOImpl();
    List<Category> categories = List.of();
    JFileChooser fileChooser = new JFileChooser();

    public WoodManagerJDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        tblDrinks.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã đồ gỗ", "Tên đồ gỗ", "Đơn giá", "Giảm giá", "Trạng thái", " "
            }
        ) {
        ...
        jLabel2.setText("Mã đồ gỗ");

        jLabel3.setText("Tên đồ gỗ");
    ...
    // Kiểm tra trống mã
    if (id.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Không được để trống mã đồ gỗ", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }

    // Kiểm tra trùng mã đồ gỗ
    for (Wood w : dao.findAll()) {
        if (w.getId().equalsIgnoreCase(id)) {
            JOptionPane.showMessageDialog(this, "Mã đồ gỗ đã tồn tại", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
    }
    // Kiểm tra tên đồ gỗ
    if (txtName.getText().trim().isEmpty()) {
        JOptionPane.showMessageDialog(this, "Không được để trống tên đồ gỗ", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }
    // Kiểm tra đơn giá
    String unitPriceText = txtUnitPrice.getText().trim();
    if (unitPriceText.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Không được để trống đơn giá", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }
    try {
        double price = Double.parseDouble(unitPriceText);
        if (price < 0) {
            JOptionPane.showMessageDialog(this, "Đơn giá phải là số dương", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
    } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(this, "Đơn giá phải là số hợp lệ (ví dụ: 120000)", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }
    // Kiểm tra danh mục
    if (cboCategories.getSelectedItem() == null) {
        JOptionPane.showMessageDialog(this, "Vui lòng chọn danh mục đồ gỗ", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }
    // Kiểm tra trạng thái
    if (!rdoAvailable.isSelected() && !rdoOutOfStock.isSelected()) {
        JOptionPane.showMessageDialog(this, "Vui lòng chọn trạng thái (Sẵn có hoặc Hết hàng)", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }
    // Nếu mọi thứ hợp lệ, thực hiện tạo mới
    this.create();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(WoodManagerJDialog.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                WoodManagerJDialog dialog = new WoodManagerJDialog(new javax.swing.JFrame(), true);
...
public Wood getForm() {
    ...
    @Override
    public void edit() {
        Wood entity = items.get(tblDrinks.getSelectedRow());
        this.setForm(entity);
