...
import poly.wood.controller.CardController;
import poly.wood.dao.CardDAO;
import poly.wood.entity.Card;
import poly.wood.dao.impl.CardDAOImpl;
...
public class CardManagerJDialog extends javax.swing.JDialog implements CardController {
    ...
    if (idText.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Không được để trống mã thẻ", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }
    ...
    try {
        ...
        if (id <= 0) {
            JOptionPane.showMessageDialog(this, "Mã thẻ phải là số nguyên dương lớn hơn 0", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
    } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(this, "Mã thẻ phải là số nguyên hợp lệ", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }

    // Kiểm tra trùng mã thẻ
    List<Card> existingCards = dao.findAll();
    for (Card c : existingCards) {
        if (c.getId() == id) {
            JOptionPane.showMessageDialog(this, "Mã thẻ đã tồn tại", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
    }
