...
import poly.wood.controller.CategoryController;
...
public class CategoryManagerJDialog extends javax.swing.JDialog implements CategoryController {
    ...
    // Kiểm tra rỗng
    if (id.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Không được để trống mã loại", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }

    if (name.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Không được để trống tên loại", "Lỗi", JOptionPane.ERROR_MESSAGE);
        return;
    }

    // Kiểm tra trùng mã loại
    List<Category> existing = dao.findAll();
    for (Category item : existing) {
        if (item.getId().equalsIgnoreCase(id)) {
            JOptionPane.showMessageDialog(this, "Mã loại đã tồn tại. Vui lòng nhập mã khác", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
    }
